...
class ReviewHistoryDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, db, item):
        ...
        ctk.CTkLabel(self, text="复习记录", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(15, 5))
        ...
        if not logs:
            ctk.CTkLabel(scroll, text="暂无复习记录", text_color="gray").pack(pady=30)
        else:
            # 表头
            header = ctk.CTkFrame(scroll, fg_color="transparent")
            header.pack(fill="x", pady=(0, 5))
            ctk.CTkLabel(header, text="日期", width=110, anchor="w", font=ctk.CTkFont(weight="bold")).pack(side="left")
            ctk.CTkLabel(header, text="复习次序", width=90, anchor="w", font=ctk.CTkFont(weight="bold")).pack(side="left")
