...
def test_stage_description_full_cycle():
    s = Scheduler()
    assert s.stage_description(1, "full") == "第1次复习"
    assert s.stage_description(6, "full") == "第6次复习"

def test_stage_description_short_cycle():
    s = Scheduler()
    assert s.stage_description(1, "short") == "第1次复习（短周期）"
    assert s.stage_description(3, "short") == "第3次复习（短周期）"
